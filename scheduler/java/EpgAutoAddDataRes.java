...
public void getResponse(HTTPurl urlData, OutputStream outStream, HashMap<String, String> headers) throws Exception
   ...
   private byte[] updateTextPage(HTTPurl urlData) throws Exception
   {
      ...
      EpgMatchList matchList = matchLists.get(name);
      ...
      EpgMatchListItem item = items.get(index);
      ...
      boolean exists = true;
      if("false".equalsIgnoreCase(urlData.getParameter("exists"))) {
		exists = false;
	  }
      ...
      String pageData =  "HTTP/1.0 200 OK\n" +
         ...
         "<html><body onLoad=\"" +
         "window.opener.location.href = '/servlet/EpgAutoAddDataRes?action=08&name=" +
   ...
   private byte[] showEditTextPage(HTTPurl urlData) throws Exception
   {
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-matchListEditText.html");
      ...
      if(item.getField() == EpgMatchListItem.FIELD_SUBTITLE) {
		buff.append(" selected");
	  }
         buff.append(">Subtitle</OPTION>\n");

      buff.append("<OPTION VALUE=\"1\"");
      if(item.getField() == EpgMatchListItem.FIELD_DESCRIPTION) {
		buff.append(" selected");
	  }
         buff.append(">Description</OPTION>\n");

      buff.append("<OPTION VALUE=\"5\"");
      if(item.getField() == EpgMatchListItem.FIELD_TITSUBDESC) {
		buff.append(" selected");
	  }
         buff.append(">Title, Subtitle or Description</OPTION>\n");

      buff.append("<OPTION VALUE=\"2\"");
      if(item.getField() == EpgMatchListItem.FIELD_CHANNEL) {
		buff.append(" selected");
	  }
      buff.append(">Channel</OPTION>\n");

      buff.append("<OPTION VALUE=\"6\"");
      if(item.getField() == EpgMatchListItem.FIELD_ACTOR) {
		buff.append(" selected");
	  }
         buff.append(">Actor</OPTION>\n");

      buff.append("<OPTION VALUE=\"3\"");
      if(item.getField() == EpgMatchListItem.FIELD_CATEGORY) {
		buff.append(" selected");
	  }
      ...
      buff.append("<OPTION VALUE=\"0\"");
      if(item.getFlags() == EpgMatchListItem.FLAG_NONE) {
		buff.append(" selected");
	  }
      buff.append(">Case Sensitive</OPTION>\n");

      buff.append("<OPTION VALUE=\"1\"");
      if(item.getFlags() == EpgMatchListItem.FLAG_CASEINSENSITIVE) {
		buff.append(" selected");
	  }
      buff.append(">Case Insensitive</OPTION>\n");

      buff.append("<OPTION VALUE=\"2\"");
      if(item.getFlags() == EpgMatchListItem.FLAG_REGEX) {
		buff.append(" selected");
	  }
   ...
   private byte[] addExcludeToMatchItem(HTTPurl urlData) throws Exception
   {
      ...
      if(match_item != null && guide_item != null)
      {
         ...
         newItem.setTextSearchData(guide_item.getName(), EpgMatchListItem.FIELD_TITLE, false, EpgMatchListItem.FLAG_CASEINSENSITIVE);
         list.add(newItem);
         store.saveMatchList(null);
      }
   ...
   {
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-watchlist-matches.html");
      ...
      if(guide_item != null)
      {
         ...
         String[] keys = matchLists.keySet().toArray(new String[0]);

         buff.append("<table width='100%'>\n");
         for (String key : keys) {
            EpgMatchList match_item = matchLists.get(key);

            if(match_item.isMatch(guide_item, wsChan))
            {
               buff.append("<tr><td nowrap>" + key + "</td><td nowrap align='right'>");
               buff.append("<a href='/servlet/EpgAutoAddDataRes?action=38&name=" + URLEncoder.encode(key, "UTF-8") + "&id=" + URLEncoder.encode(itemID, "UTF-8") + "&chan=" + URLEncoder.encode(wsChan, "UTF-8") + "'>");
               buff.append("<img src='/images/stopItem.png' width='24' height='24' title='Add program title as (Not Exists) to this item' border='0'></a> ");
               buff.append("<a href='/servlet/EpgAutoAddDataRes?action=08&name=" + URLEncoder.encode(key, "UTF-8") + "'>");
   ...
   {
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-watchlist-report.html");
      ...
      for (String itemName : watchList) {
         if(matchLists.containsKey(itemName))
         {
            EpgMatchList matcher = matchLists.get(itemName);
            ...
            for (String key : keys) {
               ...
               if (result.size() > 0)
               {
                  for (GuideItem item : result) {
                     ...
                     String startString = store.intToStr(hour) +
                        ":" + store.intToStr(startTime.get(Calendar.MINUTE)) + " " +
                        store.ampm.get(Integer.valueOf(startTime.get(Calendar.AM_PM)));

                     String dateString = store.dayName.get(Integer.valueOf(startTime.get(Calendar.DAY_OF_WEEK))) +
                        ", " + startTime.get(Calendar.DATE) + " " +
                        store.monthNameShort.get(Integer.valueOf(startTime.get(Calendar.MONTH)));
                     ...
                     buff.append("<td class='epgSearchResults'>" + key + "</td>");
   ...
   {
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-watchlist.html");
      ...
      buff.append("<table>");
      Vector<String> watchList = store.getEpgWatchList();
      for (String element : watchList) {
         if(!matchLists.containsKey(element))
         {
            store.removeEpgWatchList(element);
         }
         else
         {
            buff.append("<tr>");
            buff.append("<td>" + element + "</td>");
            buff.append("<td style='padding-left: 10px'><a href='/servlet/EpgAutoAddDataRes?action=35&watchlist=" + URLEncoder.encode(element, "UTF-8") + "'><img src='/images/delete.png' width='24' height='24' border='0'></a></td>");
            buff.append("</tr>");
            count++;
         }
      }
      ...
      buff = new StringBuffer();
      String[] items = matchLists.keySet().toArray(new String[0]);
      Arrays.sort(items);
      for (String item : items) {
         buff.append("<option value=\"" + item + "\">" + item + "</option>\n");
      }
   ...
   private byte[] textInserterPage(HTTPurl urlData) throws Exception
   {
      ...
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-textInsert.html");
      ...
      buff = new StringBuffer();
      Vector<String> progNames = new Vector<>();
      HashMap<String, HashMap<String, GuideItem>> progs = guide.getProgramList();
      keys = progs.keySet().toArray(new String[0]);

      for (String key : keys) {
         HashMap<String, GuideItem> channelProgs = progs.get(key);

         GuideItem[] items = channelProgs.values().toArray(new GuideItem[0]);

         for (GuideItem item : items) {
            if(!progNames.contains(item.getName())) {
				progNames.add(item.getName());
			}
         }
      }

      String[] names = progNames.toArray(new String[0]);
   ...
   private byte[] delMatchListToMatcher(HTTPurl urlData) throws Exception
   {
      ...
      {
         EpgMatch matcher = (store.getEpgMatchList().get(index));
         if(matcher != null)
         {
   ...
   private byte[] rescanXMLTVdata(HTTPurl urlData) throws Exception
   {
      ...
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-rescan.html");
   ...
   private byte[] showConflicts(HTTPurl urlData) throws Exception
   {
      ...
      for (AddConflictDetails conflict : conflicts) {
         conf = conflict;
   ...
   private byte[] addAutoAddItem(HTTPurl urlData) throws Exception
   {
      ...
      // make sure post task exists
      HashMap<String, TaskCommand> tasks = store.getTaskList();
      String postTask = urlData.getParameter("task");
      if (postTask == null || postTask.equalsIgnoreCase("none") || !tasks.containsKey(postTask)) {
		postTask = "";
	  }
      ...
      else
      {
         ...
         // only if new add initial match list
   ...
   {
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-auto-addform.html");
      ...
      {
         //
         //Add current match list
         //
         Vector<String> matchItems = epgMatcher.getMatchListNames();
         String[] items = matchItems.toArray(new String[0]);
         ...
         for (String name : items) {
            ...
            buff.append("<a href='/servlet/" + urlData.getServletClass() +
               "?action=17&name=" + URLEncoder.encode(name, "UTF-8") + "&index=" + indexOf +
               "' onClick='return confirmAction(\"Delete\");'><img src='/images/delete.png' border='0' alt='Delete' width='24' height='24'></a> ");

            buff.append("<a href='/servlet/" + urlData.getServletClass() +
               "?action=08&name=" + URLEncoder.encode(name, "UTF-8") +
               "'><img src='/images/edit.png' border='0' alt='Edit' width='24' height='24'></a>");
         ...
         String[] keys = matchList.keySet().toArray(new String[0]);
         Arrays.sort(keys, String.CASE_INSENSITIVE_ORDER);
         for (String key : keys) {
            String htmlName = HTMLEncoder.encode(key);

            String quotedName = key;
         ...
         template.replaceAll("$startBuff", Integer.valueOf(epgMatcher.getStartBuffer()).toString());
         template.replaceAll("$endBuff", Integer.valueOf(epgMatcher.getEndBuffer()).toString());

         if(epgMatcher.getAutoDel()) {
			template.replaceAll("$AutoDelete", "checked");
		 } else {
			template.replaceAll("$AutoDelete", "");
		 }

         template.replaceAll("$keepFor", Integer.valueOf(epgMatcher.getKeepFor()).toString());
   ...
   private String getNamePatternList(EpgMatch item)
   {
      ...
      {
         if((item == null && x == 0) || (item != null && namePatterns[x].equals(item.GetFileNamePattern()))) {
			buff.append("<option value='" + namePatterns[x] + "' selected>" + namePatterns[x] + "</option>\n");
		 } else {
			buff.append("<option value='" + namePatterns[x] + "'>" + namePatterns[x] + "</option>\n");
		 }
      }
   ...
   private String getCapturePathList(int index) throws Exception
   {
      ...
      for(int x = 0; x < capturePaths.length; x++)
      {
         ...
         if(x == index) {
			buff.append("<option value='" + x + "' selected>" + capPath + "</option>\n");
		 } else {
			buff.append("<option value='" + x + "'>" + capPath + "</option>\n");
		 }
      }
      ...
      if (id > -1)
      {
         ...
         if(item != null)
         {
            ...
            else
            {
               ...
               PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
                + File.separator + "templates" + File.separator + "epg-questionDelete.html");
   ...
   private byte[] enableEpgMatchItem(HTTPurl urlData) throws Exception
   {
      ...
      "/servlet/" + urlData.getServletClass() + "?action=01\n\n";
   ...
   private byte[] showAutoAddList(HTTPurl urlData) throws Exception
   {
      ...
      for (int x = 0; x < list.size(); x++)
      {
         ...
         {
            buff.append("<td nowrap align='center'><a href='/servlet/" + urlData.getServletClass() +
               "?action=27&index=" + x + "&enabled=false'>" +
               "<img border='0' alt='Yes' src='/images/tick.png' width='24' height='24'></a></td>");
         }
         else
         {
            buff.append("<td nowrap align='center'><a href='/servlet/" + urlData.getServletClass() +
               "?action=27&index=" + x + "&enabled=true'>" +
               "<img border='0' alt='No' src='/images/stop.png' width='24' height='24'></a></td>");
         }
         ...
         {
            String name = HTMLEncoder.encode(items.get(q));
         ...
         buff.append("<a onClick='return confirmAction(\"Delete\");' href='/servlet/" +
            urlData.getServletClass() + "?action=02&id=" + x +
            "'><img align='absmiddle' src='/images/delete.png' border='0' alt='Delete' width='24' height='24'></a> ");

         buff.append(" <a href='/servlet/" + urlData.getServletClass() + "?action=03");
         buff.append("&index=" + x +
            "'><img align='absmiddle' src='/images/edit.png' border='0' alt='Edit' width='24' height='24'></a> ");

         buff.append("<a href='/servlet/" + urlData.getServletClass() +
            "?action=23&id=" + x +
            "&dir=0'><img border='0' alt='Up' src='/images/up01.png' width='7' height='7'></a>");

         buff.append("<a href='/servlet/" + urlData.getServletClass() +
            "?action=23&id=" + x +
            "&dir=1'><img border='0' alt='Down' src='/images/down01.png' width='7' height='7'></a>");
      ...
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-auto-add.html");
      template.replaceAll("$list", buff.toString());
   ...
   private byte[] createAutoAddFromItem(HTTPurl urlData) throws Exception
   {
      ...
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-autoAddAddConfirmation.html");
   ...
   private byte[] addDayMatch(HTTPurl urlData) throws Exception
   {
      ...
      if(matchList != null)
      {
         ...
         if(sun != null && "1".equals(sun)) {
			newItem.setDayData(EpgMatchListItem.DAYS_SUN, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_SUN, false);
		 }

         String mon = urlData.getParameter("mon");
         if(mon != null && "1".equals(mon)) {
			newItem.setDayData(EpgMatchListItem.DAYS_MON, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_MON, false);
		 }

         String tue = urlData.getParameter("tue");
         if(tue != null && "1".equals(tue)) {
			newItem.setDayData(EpgMatchListItem.DAYS_TUE, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_TUE, false);
		 }

         String wed = urlData.getParameter("wed");
         if(wed != null && "1".equals(wed)) {
			newItem.setDayData(EpgMatchListItem.DAYS_WED, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_WED, false);
		 }

         String thur = urlData.getParameter("thur");
         if(thur != null && "1".equals(thur)) {
			newItem.setDayData(EpgMatchListItem.DAYS_THUR, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_THUR, false);
		 }

         String fri = urlData.getParameter("fri");
         if(fri != null && "1".equals(fri)) {
			newItem.setDayData(EpgMatchListItem.DAYS_FRI, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_FRI, false);
		 }

         String sat = urlData.getParameter("sat");
         if(sat != null && "1".equals(sat)) {
			newItem.setDayData(EpgMatchListItem.DAYS_SAT, true);
		 } else {
			newItem.setDayData(EpgMatchListItem.DAYS_SAT, false);
		 }
   ...
   private byte[] renameMatchList(HTTPurl urlData) throws Exception
   {
      ...
      if(name != null && name.trim().length() > 0)
      {
         ...
         {
            EpgMatchList matchList = matchLists.remove(oldname);
            if(matchList != null)
            {
               ...
               for (EpgMatch item : autoAddItems) {
   ...
   private byte[] showMatchList(HTTPurl urlData) throws Exception
   {
      ...
      HashMap<String, Integer> usedMatchLists = new HashMap<>();
      for (EpgMatch item : matches) {
         for (String mlName : item.getMatchListNames()) {
            if(usedMatchLists.containsKey(mlName))
            {
               Integer oldCount = usedMatchLists.get(mlName);
               usedMatchLists.put(mlName, Integer.valueOf(oldCount.intValue() + 1));
            }
            else
            {
               usedMatchLists.put(mlName, Integer.valueOf(1));
            }
         }
      }
      ...
      for (String key : keys) {
         ...
         if(usedMatchLists.containsKey(key))
         {
            count = usedMatchLists.get(key);
         }
         ...
         buff.append("<a href='/servlet/" + urlData.getServletClass() + "?action=07&name=" + URLEncoder.encode(key, "UTF-8"));
         ...
         buff.append("<a href='/servlet/" + urlData.getServletClass() + "?action=08&name=" + URLEncoder.encode(key, "UTF-8") +
            "'><img src='/images/edit.png' border='0' alt='Edit' width='24' height='24'></a>");
         buff.append("</td>");
         buff.append("</tr>\n");
      }
      ...
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-matchLists.html");
   ...
   private byte[] editMatchList(HTTPurl urlData) throws Exception
   {
      ...
      PageTemplate template = new PageTemplate(store.getProperty("path.httproot")
        + File.separator + "templates" + File.separator + "epg-matchListEdit.html");
      ...
      {
         EpgMatchListItem item = items.get(x);
         buff.append("<tr>");
         ...
         if(item.getType() == EpgMatchListItem.TYPE_TEXT)
         {
            ...
            else if(item.getField() == EpgMatchListItem.FIELD_ACTOR)
            {
               buff.append("actor");
            }
            ...
            if(item.getFlags() == EpgMatchListItem.FLAG_NONE) {
				buff.append(" (Case)");
			} else if(item.getFlags() == EpgMatchListItem.FLAG_CASEINSENSITIVE) {
				buff.append(" (No Case)");
			} else if(item.getFlags() == EpgMatchListItem.FLAG_REGEX) {
				buff.append(" (RegEx)");
			}
         ...
         else if(item.getType() == EpgMatchListItem.TYPE_SPAN)
         {
            ...
            if (item.getSpanType() != 0) {
				buff.append("</b> (Duration)");
			} else {
				buff.append("</b> (Start)");
			}
            buff.append("</td>");
         ...
         else if(item.getType() == EpgMatchListItem.TYPE_DAYS)
         {
            ...
            if(item.getDayValue(EpgMatchListItem.DAYS_SUN)) {
				buff.append(" Sun ");
			}

            if(item.getDayValue(EpgMatchListItem.DAYS_MON)) {
				buff.append(" Mon ");
			}

            if(item.getDayValue(EpgMatchListItem.DAYS_TUE)) {
				buff.append(" Tue ");
			}

            if(item.getDayValue(EpgMatchListItem.DAYS_WED)) {
				buff.append(" Wed ");
			}

            if(item.getDayValue(EpgMatchListItem.DAYS_THUR)) {
				buff.append(" Thur ");
			}

            if(item.getDayValue(EpgMatchListItem.DAYS_FRI)) {
				buff.append(" Fri ");
			}

            if(item.getDayValue(EpgMatchListItem.DAYS_SAT)) {
				buff.append(" Sat ");
			}
         ...
         else if(item.getType() == EpgMatchListItem.TYPE_FLAG)
         {
            ...
            if(flagID == EpgMatchListItem.ITEM_FLAG_REPEAT) {
				buff.append(" Repeat ");
			} else if(flagID == EpgMatchListItem.ITEM_FLAG_LIVE) {
				buff.append(" Live ");
			} else if(flagID == EpgMatchListItem.ITEM_FLAG_HD) {
				buff.append(" High Definition ");
			} else if(flagID == EpgMatchListItem.ITEM_FLAG_WS) {
				buff.append(" Wide Screen ");
			} else if(flagID == EpgMatchListItem.ITEM_FLAG_CC) {
				buff.append(" Closed Captions ");
			} else if(flagID == EpgMatchListItem.ITEM_FLAG_PREMIERE) {
				buff.append(" Premiere ");
			}

            if(item.getFlagValue()) {
				buff.append("= TRUE");
			} else {
				buff.append("= FALSE");
			}
         ...
         buff.append("<a href='/servlet/" + urlData.getServletClass() +
            "?action=13&name=" + URLEncoder.encode(name, "UTF-8") + "&index=" + x +
            "' onClick='return confirmAction(\"Delete\");'><img src='/images/delete.png' border='0' alt='' title='Delete' width='24' height='24'></a> ");

         if(item.getType() == EpgMatchListItem.TYPE_TEXT)
         {
            buff.append("<a href='#' onclick='editTextType(\"" + URLEncoder.encode(name, "UTF-8") + "\", " + x + ");'>" +
               "<img src='/images/edit.png' border='0' alt='' title='Edit' width='24' height='24'></a>");
         }
      ...
      if("1".equals(urlData.getParameter("show")))
      {
         ...
         for (String key : keys) {
            ...
            if (result.size() > 0)
            {
               ...
               buff.append("<span class='areaTitle'>" + key + "</span><br>\n");
               buff.append("</td></tr\n");
   ...
   private String getTaskList(String current)
   {
      ...
      if(selectedTask.length() == 0) {
		buff.append("<option value='none' selected>none</option>\n");
	  } else {
		buff.append("<option value='none'>none</option>\n");
	  }
   ...
   private String getCaptureTypes(int current)
   {
      ...
      for (CaptureCapability capability : capabilities) {
         ...
         if(capType == capability.getTypeID()) {
			buff.append(" selected");
		 }
