...
int CStreamScan::findProgramID(int pid)
{
	for (std::map<int, std::vector<CStreamInfo> >::iterator mi = programs.begin(); mi != programs.end(); mi++)
	{
		...
		for(std::vector<CStreamInfo>::size_type y = 0; y < pids.size(); y++)
		{
			if(pids.at(y).getID() == pid && pids.at(y).isMediaType() == true)
			{
				log("Found Pid (%d) of type (%d) in Program (%d)\r\n", pid, pids.at(y).getType(), progID);
				return progID;
			}
		}
	}
...
int CStreamScan::scanForProgs(IBaseFilter *psiFilter)
{
	...
	HRESULT hr = S_OK;
	int retries = 0;
	while(programs.size() == 0 && retries++ < 3)
	{
		log("Starting Mpeg2 Table Parsing (try:%d)\r\n", retries);

		// Get program map timeout = 5 sec
		hr = parseMpeg2Table(pMpeg2Data, 0x0000, 0x00, 5000);

		if(FAILED(hr))
		{
			log("Error Parsing Sections and Tables (0x%x)\r\n", hr);
			Sleep(2000);
		}
	}
...
HRESULT CStreamScan::parseMpeg2Table(IMpeg2Data* pMpeg2Data, PID pid, TID tid, DWORD dwTimeout)
{
	...
	if (pMpeg2Data != NULL)
	{
		...
		if(SUCCEEDED(hr))
		{
			...
			{
				log("Error getting number of sections (0x%x)\r\n", hr); 
			}
		}
		else
		{
			log("Timeout getting table (pid=%.4x, tid=%.2x)\r\n", pid, tid);
		}
	}
...
HRESULT CStreamScan::parseMpeg2Section(IMpeg2Data* pMpeg2Data, SECTION *pSection, DWORD dwPacketLength)
{ 
    if (!pSection)
        return E_POINTER;
		...
		case 0x00:	//PAT
			{
				...
				while (i < (pHeader->SectionLength - 5))
				{
					...
					//printf("programNumber= %d programId = %d total numb = %d\r\n", programNumber, programId, number_of_programs);
					//printf("programNumber= %d programId = %d\r\n", programNumber, programId);
        ...
        case 0x02:	//PMT
			{
				...
				if (ProgIdSave != 0)
				{
					...
					//printf("ProgNumbSave= %d ProgIdSave = %d\r\n", ProgNumbSave, pgm_pids[ProgNumbSave]);
				} 
				...
				if ((NTOH_S(WORD_VALUE(pSection,8)) & 0x1fff) != 0)
				{
					...
					//printf("ProgNumbSave= %d pcrpid = %d\r\n", ProgNumbSave, pcr_pids[ProgNumbSave]);
				} 
				
//				if (ProgChanSave != 0)
//				{
					//printf("ProgNumbSave= %d ProgChanSave = %d\r\n", ProgNumbSave, ProgChanSave);
//				}
				...
				if (private_pid != -1)
				{
					if (audio_pid == -1 && ac3aud_pid != -1)
					{
						...
						info.setType(TYPE_AUDIO_AC3);
						programs[ProgNumbSave].push_back(info);
						printf("Private01 Add Stream %d %d(TYPE_AUDIO_AC3)\r\n", ProgNumbSave, private_pid);
					} 
					else if (ac3aud_pid == -1 && telex_pid != -1)
					{
						...
						info.setType(TYPE_AUDIO_AC3);
						programs[ProgNumbSave].push_back(info);
						printf("Private02 Add Stream %d %d(TYPE_AUDIO_AC3)\r\n", ProgNumbSave, private_pid);
					}
					else if (ac3aud_pid == -1 && telex_pid == -1)
					{
						CStreamInfo info;
						...
						info.setType(TYPE_AUDIO_AC3);
						programs[ProgNumbSave].push_back(info);
						printf("Private03 Add Stream %d %d(TYPE_TELETEXT)\r\n", ProgNumbSave, private_pid);
					}
				}
				*/
