...
void openLogFile(char *logFile)
{
	...
	int createResult = SHCreateDirectoryEx(NULL, logPath.c_str(), NULL);
	...
	char name[256];
	StringCchPrintf(name, 256, "\\%s%.4d%.2d%.2d-%.2d%.2d%.2d-%.3d.log", logFile, st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
